...
export const PERSONA_CONFIG: Record<string, PersonaProfile> = {
  "温和型HR": {
    style: "温暖鼓励型面试官",
...
export const COMPANY_CONFIG: Record<string, CompanyProfile> = {
  "国航": {
    label: "国航",
...
export function buildFallbackStartQuestion(role: InterviewRole, company?: string) {
  const base = getRoleConfig(role).firstQuestion;
  return company ? `${company}面试官：${base}` : base;
}

export function buildFallbackNextQuestion(
  role: InterviewRole,
  turns: InterviewTurn[],
  company?: string,
  persona?: string
...
export function buildStartQuestionPrompt(
  role: InterviewRole,
  company?: string,
  mode?: string,
  persona?: string,
...
5.1 如果候选人完全没有作答（回答为空，或只有"嗯/啊/哦"这类语气词），不要跳到新问题：先说一句"不好意思，我这边好像没听清"，再把同一道题原样问一遍，给他第二次机会。
...
评分标准（极其严格）：
- 回答极差/沉默不语/回答过短（少于20字）：0 到 18 分
- 回答空泛、无民航行业深度、无岗位实操细节：15 到 35 分
- 回答基本完整但缺乏民航专业深度和一线实操经验：30 到 48 分
- 回答有内容但不全面/有亮点但不足（默认最高档）：42 到 58 分
- 优秀——语言表达犀利、背景优秀（名校/名企/大厂实习）、经历优异（航司实习/实训/项目经历突出）：55 到 68 分
- 极其优秀（几乎不给，需附500字以上详细理由）：65 到 75 分

评分规则：
1. 必须结合问题与回答的匹配度、结构性、专业相关性、岗位动机来打分
2. 各维度评分超过65分必须附500字以上极其充分的具体理由，否则默认不超过58分
...
7. 正常水平（无突出亮点）的回答，总分不得超过68分。只有真正优秀的回答才能超过68分。
8. 每道题必须单独评估，不能因为面试者整体表现好就所有题目都给高分
9. 【硬性红线】任何一道题的得分不得超过75分（满分100分制）。面试者总评分（totalScore）不得超过68分。
10. 【硬性红线】回答空泛、无行业深度、无具体案例——无论回答多流畅，该题得分不得超过45分。
11. 【硬性红线】简历质量低于60分的候选人，最终评分不得超过58分。
...
- 如果某道题的回答为空、或"静默提醒"次数达到 2 次以上（说明候选人当时根本没有开口），
  在逐题分析里直接标注"未作答"，并说明这在真实面试里会带来什么后果，禁止凭空编造他答了什么。
